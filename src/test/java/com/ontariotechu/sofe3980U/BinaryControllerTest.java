...
public class BinaryControllerTest {
    ...
    public void postParameterMultiply() throws Exception {
        this.mvc.perform(post("/").param("operand1", "101").param("operator", "*").param("operand2", "11"))
                .andExpect(status().isOk())
                .andExpect(view().name("result"))
                .andExpect(model().attribute("result", "1111"))  // 5 * 3 = 15 (1111 in binary)
                .andExpect(model().attribute("operand1", "101"));
    }

    // Bitwise AND Test
    @Test
    public void postParameterBitwiseAnd() throws Exception {
    ...
    // Bitwise OR Test
    @Test
    public void postParameterBitwiseOr() throws Exception {
        this.mvc.perform(post("/").param("operand1", "1101").param("operator", "|").param("operand2", "1011"))
                .andExpect(status().isOk())
                .andExpect(view().name("result"))
                .andExpect(model().attribute("result", "1111"))  // 1101 | 1011 = 1111
                .andExpect(model().attribute("operand1", "1101"));
    }
}